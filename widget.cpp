...
void Widget::paintEvent(QPaintEvent *) //绘图事件，画棋盘，画棋子
{
    QPainter p(this); //创建一个画家，指定窗口wie画布
...
void Widget::mousePressEvent(QMouseEvent *e)//鼠标按下事件，获取当前点击的坐标，下子坐标
{
    if(e->button() == Qt::LeftButton) //如果是鼠标左键
    {
...
void Widget::mouseMoveEvent(QMouseEvent *e) //鼠标移动事件，移动窗口
{
    //如果按住的是左键，需要移动窗口
...
void Widget::on_toolButtonBegin_clicked()
{
    //初始化棋盘状态
    initChess();
